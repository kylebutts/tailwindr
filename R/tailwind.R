...
#' @export
#'
tailwind = function(highlight = "zenburn", slim_css = FALSE, self_contained = FALSE, css = NULL, clean_supporting = TRUE, template = NULL, ...) {
    ...
    post_processor <- function(metadata, input_file, output_file, clean, verbose) {
        ...
        # tailwind_prose.css
        if(!file.exists(file.path(output_dir, "tailwind.css"))) {
            cat('
                @tailwind base;
                @tailwind components;
                @tailwind utilities;
                ', file = file.path(output_dir, "tailwind.css"))
        }
        ...
        command <- paste0(
            ...
            "cat ", file.path(output_dir, "tailwind.css"), " ", paste(css, collapse = " "), " | ",
        ...
        if(clean_supporting) {
            file.remove(
                ...
                file.path(output_dir, "tailwind.css")
            )
        }
